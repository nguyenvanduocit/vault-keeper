...
import { fenceSelect } from "../helpers/fence.js";
...
export function validateConfig(param, path, issues) {
  if (param.content && typeof param.content === "object") {
    checkUnknownKeys(param.content, CODE_CONTENT_INNER_KEYS, "'code.content'", path, issues);
    if (param.content.pattern) {
      try {
        new RegExp(param.content.pattern);
      } catch {
        issues.push(issue(
          "error",
          path,
          `Invalid regex in code.content.pattern of '${path}': ${param.content.pattern}`,
