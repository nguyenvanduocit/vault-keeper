...
/**
 * Validate section content against section-rules primitives
 * (table, list, code, formula). `frontmatter` is passed through so
 * the `formula` primitive can evaluate against frontmatter values.
 */
function validateSectionContent(rules, docNode, path, issues, frontmatter = {}) {
  ...
  if (rules.list) {
    ...
    // Anchor list-level issues at the list node. Per-item issues keep
    // their own item.line set by the primitive.
  ...
  if (rules.code) {
    ...
    // Anchor cardinality issues at the first fence in the section,
    // falling back to the section heading line when no fence is
    // present. Per-fence `code-content-mismatch` issues already carry
    // their own bodyLine — do NOT overwrite those.
  ...
  // formula — evaluates against frontmatter values. References inside
  // the expression map directly to frontmatter field names.
  if (rules.formula) {
    const ctx = { field: path, severity, message: rules.message };
