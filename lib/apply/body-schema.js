 ...
 * to the registered body-section primitives to validate section
 * content.
 *
 ...
 * @returns {Promise<BodyIssue[]>}
 */
export async function applyBodySchema(templateBodySchema, docMarkdownBody, docMeta, frontmatter = {}) {
  ...
  await validateChildren(effectiveSchema, effectiveDocNodes, [], issues, frontmatter);
 ...
 */
async function validateChildren(schemaNodes, docNodes, ancestorPath, issues, frontmatter) {
  ...
  for (const schema of nonRepeatableSchemas) {
    ...
    await validateSection(schema, docNode, ancestorPath, issues, frontmatter);
  }
  ...
  for (const schema of repeatableSchemas) {
    ...
    for (const docNode of matchingDocNodes) {
      ...
      if (schema.children && schema.children.length > 0) {
        await validateChildren(schema.children, docNode.children, [...ancestorPath, itemLabel], issues, frontmatter);
      }

      // Validate section content against registered section-rules primitives
      await validateSectionContent(rules, docNode, itemPath, issues, frontmatter);
    }
  }
}
 ...
 * into its children.
 */
async function validateSection(schema, docNode, ancestorPath, issues, frontmatter) {
  const label = headingLabel(schema.depth, schema.text);
  const path = headingPath(ancestorPath, label);
  const rules = schema.sectionRules || {};

  await validateSectionContent(rules, docNode, path, issues, frontmatter);

  if (schema.children && schema.children.length > 0) {
    await validateChildren(schema.children, docNode.children, [...ancestorPath, label], issues, frontmatter);
  }
}
 ...
 */
async function validateSectionContent(rules, docNode, path, issues, frontmatter = {}) {
  const severity = rules.severity || "error";
