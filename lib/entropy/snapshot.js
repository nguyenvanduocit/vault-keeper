...
export function diffSnapshots(before, after) {
  ...
  const dim = (name) => {
    const b = before.dimensions[name].score;
    const a = after.dimensions[name].score;
    return {
      before: b,
      after: a,
      // null - 85 silently coerces to -85 in JS; propagate null through
      // so the CLI's signed() formatter can render 'n/a' for degenerate
      // snapshots (empty vault → overall_score: null).
      delta: (b === null || a === null) ? null : a - b,
    };
  };
  const ob = before.overall_score;
  const oa = after.overall_score;
  return {
    elapsed_days: elapsedMs / (1000 * 60 * 60 * 24),
    overall_delta: (ob === null || oa === null) ? null : oa - ob,
    dimensions: {
      schema: dim('schema'),
      vocab: dim('vocab'),
