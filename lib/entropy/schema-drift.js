...
export function measureSchemaDrift(docs, templates) {
  ...
  for (const [templatePath, templateDocs] of docsByTemplate) {
    ...
    for (const [fieldName, spec] of Object.entries(fields)) {
      ...
      // Detailed value reporting (value_distribution, orphan_values,
      // entropy_norm) is only meaningful for enum-constrained fields, where the
      // value set is BOUNDED. For free / patterned / temporal / identity fields
      // (timestamps, titles, owners) every value is distinct by design, so a
      // full value histogram is noise, not drift. Non-enum fields report only
      // drift_score + a distinct_values count.
      ...
      perTemplate[templatePath].fields[fieldName] = {
        ...
        // Full histogram only for enums; non-enum fields just carry the count.
        value_distribution: isEnum ? Object.fromEntries(valueCounts) : {},
        distinct_values: valueCounts.size,
        drift_score,
        orphan_values,
        entropy_norm,
      };
